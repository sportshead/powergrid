...
func UpdateCommands(list []interface{}) {
	commands, err := session.ApplicationCommands(env.DiscordApplicationID, env.DiscordGuildID)
	if err != nil {
	...
	for _, oldCommand := range commands {
		...
		if i == -1 {
			err = session.ApplicationCommandDelete(oldCommand.ApplicationID, oldCommand.GuildID, oldCommand.ID)
			if err != nil {
		...
		if oldCommand.Type != newCommand.Type || !reflect.DeepEqual(oldCommand, newCommand) {
			...
			edited, err = session.ApplicationCommandEdit(oldCommand.ApplicationID, oldCommand.GuildID, oldCommand.ID, newCommand)
			if err != nil {
	...
	for _, i := range list {
		...
		created, err = session.ApplicationCommandCreate(env.DiscordApplicationID, env.DiscordGuildID, newCommand)
		if err != nil {
