...
# ./github.com/sportshead/powergrid/coordinator resolves to ./././../coordinator, or ./.
ln -s . github.com
ln -s . sportshead
ln -s .. powergrid
trap "rm github.com && rm sportshead && rm powergrid" EXIT

kube::codegen::gen_helpers \
  --input-pkg-root github.com/sportshead/powergrid/coordinator/pkg/apis \
  --boilerplate /dev/null \
  --output-base "${PKG_ROOT}"

kube::codegen::gen_client \
  --input-pkg-root github.com/sportshead/powergrid/coordinator/pkg/apis \
  --output-pkg-root github.com/sportshead/powergrid/coordinator/pkg/generated \
  --boilerplate /dev/null \
  --output-base "${PKG_ROOT}" \
  --with-watch \
  --with-applyconfig
