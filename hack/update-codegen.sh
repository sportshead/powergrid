...
# ./github.com/sportshead/coordinator resolves to ././../powergrid, or ./.
ln -s . github.com
ln -s .. sportshead
trap "rm github.com && rm sportshead" EXIT

kube::codegen::gen_helpers \
  --input-pkg-root github.com/sportshead/powergrid/pkg/apis \
  --boilerplate /dev/null \
  --output-base "${PKG_ROOT}"

kube::codegen::gen_client \
  --input-pkg-root github.com/sportshead/powergrid/pkg/apis \
  --output-pkg-root github.com/sportshead/powergrid/pkg/generated \
  --boilerplate /dev/null \
  --output-base "${PKG_ROOT}" \
  --with-watch \
  --with-applyconfig
